...
а не два самостоятельных вопроса. Счётчик выпуска: `05/32`.
...
| Где ловить исключение | 19:23–20:28 | Михаил не дал ясного ответа | сохранить реплики и показать помеченное `Уточнение ответа` |
...
Исходный ответ продолжает звучать без замены. На экране появляется небольшая,
но заметная метка `Уточнение ответа` и проверенная формулировка:
...
|---|---|---|---|---|
| Базовый экран | 18:15–18:45 | вводная про уровень вопроса, книги и слои | сохранить атмосферу без лишнего учебного слоя | Base scene |
| 1. Вопрос | 18:45–18:59 | открытый вопрос про DDD и слои | удержать обе части темы | Question |
...
| 4. Уточнение и границы | 19:29–20:52 | исходное обсуждение, затем объяснение интервьюера | сначала дать короткий критерий перехвата, затем раскрыть catch/translate/response flow | Correction + layer flow |
...
```text
┌──────────────────────────────────────────────────────────────┐
│ 05/32  Исключение должно сообщать смысл сбоя                 │
├──────────────────────────────┬───────────────────────────────┤
...
```text
┌──────────────────────────────────────────────────────────────┐
│ 05/32  Ловим там, где можем принять решение                  │
│                                      Уточнение ответа         │
├──────────────────────────────────────────────────────────────┤
│ Payment provider                                             │
│ GatewayTimeout                                               │
...
- `18:45` — появляется основной вопрос; активен интервьюер.
- `18:59` — generic exception слева, затем справа типы по мере ответа Михаила.
- `19:23` — отдельный крупный follow-up «Где ловить?».
- `19:29` — поверх исходного разговора появляется метка `Уточнение ответа` и
  короткий критерий: ловим там, где можем принять решение.
...
- Визуалы покрывают непрерывный фрагмент `18:15–20:52` и имеют отдельную
...
- Метка `Уточнение ответа` обязательна и остаётся в safe-зоне.
...
- Исправление выполняется экранным слоем `Уточнение ответа`; досъёмка,
...
- [x] Недосказанный ответ исправляется помеченным экранным уточнением.
