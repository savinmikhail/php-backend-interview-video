...
- Вопрос звучит в `38:38–38:52` исходника, что соответствует
  `24:52–25:06` review-таймлайна; точную границу сверить по оригинальному
  звуку при реализации.
...
|---|---|---|---|---|---|
| 1 · Вопрос | `38:38–38:52` | `24:52–25:06` | интервьюер проговаривает вопрос | удержать формулировку без преждевременного ответа | Question |
| 2 · Один `flush()` | `38:52–39:13` | `25:06–25:27` | Михаил начинает отвечать про raw SQL | дать отсутствующий базовый контраст: ORM-записи уже объединены | Code + transaction trace |
| 3 · Явная транзакция | `39:13–40:01` | `25:27–26:15` | ответ про SQL/DBAL и уточнение вопроса | показать конкретные случаи ручной границы | Code + compact cases |
...
```text
┌──────────────────────────────────────────────────────────────┐
│ 16/32  Обычно beginTransaction() не нужен                    │
├─────────────────────────────┬────────────────────────────────┤
...
```text
┌──────────────────────────────────────────────────────────────┐
│ 16/32  Когда открываем транзакцию явно?     Уточнение ответа │
├────────────────────────────────┬─────────────────────────────┤
