...
- Пример с `enum` удерживается до `21:23`; затем до начала вопроса про
  DI-контейнер в `21:37` остаётся базовый экран с аватарами.

## Редакционная единица

Это один короткий вопрос, хотя в учебном индексе он оказался далеко от
соседних вопросов. Счётчик выпуска: `06/30`.
...
|---|---|---|---|---|
| 1. Вопрос и пример | 20:59–21:23 | вопрос про назначение `enum` и завершение PHP-блока | дать зрителю время прочитать оба кейса | Code comparison |
| Базовый экран | 21:23–21:37 | переход к Symfony-блоку | не конкурировать с вводной репликой | Base scene |
...
```text
┌──────────────────────────────────────────────────────────────┐
│ 06/30  Для чего и когда использовать enum?                   │
├──────────────────────────────┬───────────────────────────────┤
