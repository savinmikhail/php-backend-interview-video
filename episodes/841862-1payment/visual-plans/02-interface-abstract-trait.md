...
- Фрагмент: `15:45–16:42` исходного интервью.
...
```text
┌──────────────────────────────────────────────────────────────┐
│ 02/30  Интерфейс, abstract class, trait — что для чего?      │
├──────────────────────────────────────────────────────────────┤
│                     interface Notifier                       │
...
- `16:24` — один переход к `trait Timestampable`; это `02:38` сокращённого
  review-таймлайна.
- `16:28` — trait подключается к `Order` и `Article`.
- `16:36` — остаётся собранная схема без новой информации до `16:42`, когда
  сразу начинается следующий вопрос.
...
- Фрагмент `15:45–16:42` укладывается в один Short без искусственного деления.
...
Сохраняем формат `02/30`, несмотря на объединение трёх учебных пунктов в один
реальный вопрос интервью. Финальное количество монтажных единиц — 30.
