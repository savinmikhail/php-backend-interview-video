...
```text
┌──────────────────────────────────────────────────────────────┐
│ 02/58  Интерфейс, abstract class, trait — что для чего?      │
├──────────────────────────────────────────────────────────────┤
│                     interface Notifier                       │
...
Сохраняем текущий формат `02/58`, несмотря на объединение трёх учебных пунктов
в один реальный вопрос интервью. После подготовки всех вопросов пересчитываем
монтажные единицы и одной правкой заменяем общее количество, например `58` на
`45`. До этого число `58` считается временным.
