...
Счётчик выпуска: `03/32`; финальное общее количество — 32.
...
| Изменение свойства внутри метода видно снаружи | 16:56–17:03, 17:43–17:49 | корректно | показать код при первом объяснении; повторную реплику оставить без инфографики |
...
| Переприсваивание параметра не заменяет объект у вызывающего кода | не звучит | важное следствие | не добавлять: для этого фрагмента это лишний смысловой слой |
...
- Учебную инфографику поверх реплик `17:19–17:49`: этот кусок работает на
  атмосферу, а техническая мысль про видимую снаружи мутацию уже показана.
- Дополнительное сравнение мутации и локального переприсваивания: оно не
  звучит в интервью и повторно возвращает зрителя к уже закрытому вопросу.
...
| 2. Мутация без `&` | 16:48–17:05 | обсуждается видимая снаружи мутация | показать минимальный вызов и результат без схемы внутреннего устройства | Code + result |
| 3. Identifier, не alias | 17:05–17:19 | Михаил уточняет механизм | показать две `zval`, содержащие один object handle | Internals diagram |
| Пауза в инфографике | 17:19–17:49 | живая реплика и повтор практического смысла | сохранить атмосферу: только фон и аватары | Base scene |

После состояния 3 центральная карточка растворяется. С `17:19` по `17:49`
остаются фиолетовый фон и говорящие аватары.
...
```text
┌──────────────────────────────────────────────────────────────┐
│ 03/32  Что происходит при передаче объекта в метод?          │
├──────────────────────────────────────────────────────────────┤
...
- `17:05` — код сменяется схемой двух `zval` с handle `#1434` и проверкой через
  `spl_object_id()`.
...
- Вертикальная последовательность: вопрос → мутация без `&` → две `zval` и один
  object handle → атмосферная пауза.
...
- Монтаж: исходный диалог не режем; на `17:19–17:49` скрываем центральную
  инфографику.
